...
@artefacts({PickleSerializer: ["sklearn_preproc"]})
class MixedModel:
    ...
    def fit(self, x_train, y_train, epochs) -> None:
        ...
        x_train = self.scaler.fit_transform(
            np.reshape(x_train, (x_train.shape[0], 28 * 28))
        )
        x_train = np.reshape(x_train, (x_train.shape[0], 28, 28))
        self.model.fit(x_train, y_train, epochs)

    def predict(self, x_test):
        x_test = self.scaler.fit_transform(
            np.reshape(x_test, (x_test.shape[0], 28 * 28))
        )
